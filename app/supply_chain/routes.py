...
def index():
    ...
    current_requests = Request.query \
        ...
        .add_columns(Request.name, Request.status, Affected.first_name, Affected.last_name, Address.city, 
                    Address.street, Address.voivodeship, DonationType.type) \
        .all()
...
def add_data():
    ...
    if len(address_result) != 0:
        ...
        donor1 = Donor(
            ...
            user_id=User.query.filter(User.email == "john.doe@example.com").first().id
        )
        ...
        organization_charity_campaign_1 =  OrganizationCharityCampaign(
            ...
            organization_id=Organization.query.filter(Organization.organization_name == 'FUNDACJA OJCA RYDZYKA').first().id
        )
